...
class ResNet(nn.Module):
    ...
    def __init__(
        ...
        self.name = f"resnet{sum(layers)*2+2}"
        self.inplanes = 64
        self.dilation = 1
        ...
        self.conv1 = nn.Conv2d(1, self.inplanes, kernel_size=(1, 5), stride=(1, 2), padding=(1, 3), bias=False)
        if has_bn:
...
class BPHSplit(nn.Module):
    def __init__(self, base, proj, head):
        super(BPHSplit, self).__init__()
...
if __name__=='__main__':
    ...
    model_size_MB = model_size / (1024 * 1024)  # 转换为 MB
