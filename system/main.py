...
from flcore.servers.serverFIDS_CL import FIDS_CL
...
def run(args):
    ...
    for i in range(args.prev, args.times):
        ...
        if args.algorithm == "FIDS_CL":
            args.head = copy.deepcopy(args.model.fc)
            args.model.fc = nn.Identity()
            args.model = BaseHeadSplit(args.model, args.head)
            server = FIDS_CL(args, i)
...
if __name__ == "__main__":
    ...
    parser.add_argument('-Fstart', '--f1_score_start_epoch', type=int, default=25)
    parser.add_argument('-Fend', '--f1_score_end_epoch', type=int, default=50)
    parser.add_argument('-Lstart', '--loss_score_start_epoch', type=int, default=25)
    parser.add_argument('-Lend', '--loss_score_end_epoch', type=int, default=50)
    parser.add_argument('-al', '--alpha', type=float, default=0.8)
    parser.add_argument('-th', '--acc_thre', type=float, default=0.75, help='con1 parameters')
    parser.add_argument('-N', '--num_low_classes', type=int, default=3)
    # parser.add_argument('-MH', '--model_heter', type=bool, default=False)
    # parser.add_argument('-mcd','--model_cd', type=float, nargs='+')
